...
- Runtime execution happens via `scripts/bitlesson-select.sh`, which routes to Codex CLI (`codex exec`) for `gpt-*` models or Claude CLI (`claude --print`) for Claude models (`fable`, `haiku`, `sonnet`, `opus`), based on the configured `bitlesson_model`.
