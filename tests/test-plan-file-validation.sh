...
rm -rf "$TEST_DIR/.humanize/rlcr" 2>/dev/null || true
set +e
RESULT=$("$PROJECT_ROOT/scripts/setup-rlcr-loop.sh" --codex-model "gpt-5.6:high#comment" "plans/test-plan.md" 2>&1)
EXIT_CODE=$?
set -e
...
set +e
RESULT=$("$PROJECT_ROOT/scripts/setup-rlcr-loop.sh" --codex-model "gpt-5.6:medium" "plans/test-plan.md" 2>&1)
EXIT_CODE=$?
set -e
