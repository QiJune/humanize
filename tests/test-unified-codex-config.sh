...
if ! command -v jq >/dev/null 2>&1; then
    ...
    assert_eq "default_config.json: codex_model is gpt-5.6" \
        "gpt-5.6" "$(jq -r '.codex_model' "$DEFAULT_CONFIG")"
...
if [[ ! -f "$CONFIG_LOADER" ]]; then
    ...
    assert_eq "default-only: codex_model defaults to gpt-5.6" \
        "gpt-5.6" "$(get_config_value "$merged" "codex_model")"
...
if [[ ! -f "$LOOP_COMMON" ]]; then
    ...
    assert_eq "invalid config: codex_model falls back to gpt-5.6" \
        "gpt-5.6" "$(echo "$result_line" | cut -d':' -f2 | cut -d'|' -f1)"
    ...
    for invalid_model in haiku false claude-3; do
        ...
        assert_eq "non-Codex config ($invalid_model): codex_model falls back to gpt-5.6" \
            "gpt-5.6" "$(echo "$result_line" | cut -d':' -f2 | cut -d'|' -f1)"
...
BARE_EOF
    ...
    assert_eq "bare state: falls back to DEFAULT_CODEX_MODEL (gpt-5.6)" \
        "gpt-5.6" "$(echo "$result" | cut -d'|' -f1)"
...
STALE_EOF
    ...
    assert_eq "stale state: STATE_CODEX_MODEL still parsed (gpt-5.6)" \
        "gpt-5.6" "$(echo "$result" | cut -d'|' -f1)"
...
model_with_shell="gpt-5.6;rm-rf"
